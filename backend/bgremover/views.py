...
class BGRemoverThrottle(AnonRateThrottle):
    rate = '5/min'
...
@api_view(['POST'])
@throttle_classes([BGRemoverThrottle])
...
def remove_background(request):
    ...
    if not token:
        return Response({'error': 'Background removal service not configured.'}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
    ...
    if resp.status_code != 200:
        return Response(
            {'error': f'Background removal failed ({resp.status_code}). Please try again.'},

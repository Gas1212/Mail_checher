...
const STORAGE_KEY = 'sugesto_credits';

export function useCredits(toolName: string, isAuthenticated: boolean = false) {
  ...
  useEffect(() => {
    const loadCredits = () => {
      try {
        if (!isAuthenticated) {
          // For non-authenticated users, use old free trial system
          setIsLoading(false);
          return;
        }
        ...
        if (stored) {
          ...
          // Check if we need to reset (e.g., monthly reset)
          ...
          if (daysSinceReset >= 30) {
            ...
            localStorage.setItem(`${STORAGE_KEY}_${toolName}`, JSON.stringify(resetData));
            ...
            const updatedData = { ...data, requestHistory: cleanedHistory };
          ...
          localStorage.setItem(`${STORAGE_KEY}_${toolName}`, JSON.stringify(initialData));
  ...
  useEffect(() => {
    if (!isAuthenticated || credits.requestHistory.length === 0) return;
  ...
  }, [credits, toolName, isAuthenticated]);
